...
public class StorageLuceneTest
{
	...
	public void simpleTest() throws ParseException {
		...
		assertEquals(expected, storageLucene.findSet(new QueryParser("text", new WhitespaceAnalyzer()).parse("value0")));
		expected = new EWAHBitSet().set(1).set(2);
		assertEquals(expected, storageLucene.findSet(new QueryParser("text", new WhitespaceAnalyzer()).parse("value1")));
	}
	...
	public void wildcardSearchTest() throws ParseException {
		...
		assertEquals(expected, storageLucene.findSet(new QueryParser("text", new WhitespaceAnalyzer()).parse("v?lu*")));
	}
	...
	public void regExpSearchTest() throws ParseException {
		...
		BitSet expected = new EWAHBitSet().set(0);
		assertEquals(expected, storageLucene.findSet(new QueryParser("text", new WhitespaceAnalyzer()).parse("/[fvs]alue./")));
	}
}
