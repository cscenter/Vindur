...
//    private static final int[] costBounds = {3000, 5000, 7500, 10000, 15000, 25000, 200000};
...
//    };
//
//    private static final Manufacturer[][] smartManufacturers = new Manufacturer[][]{
//            //1st cat: [3000, 5000]
//            {
...
//
//    };
//    private static final Manufacturer[][] phoneManufacturers = new Manufacturer[][]{
//            //1st cat: [3000, 5000]
//            {
...
//            Manufacturer[] targetManufactorer = (smart ? smartManufacturers[category] : phoneManufacturers[category]);
//            double chance = Math.random();
...
//                new PhoneCost(costBounds[2], costBounds[3], 0.55), new PhoneCost(costBounds[3], costBounds[4], 0.75),
//                new PhoneCost(costBounds[4], costBounds[5], 0.9), new PhoneCost(costBounds[5], costBounds[6], 1.0)
//        };
//        private final PhoneCost[] phoneCosts = new PhoneCost[]{
//                new PhoneCost(costBounds[0], costBounds[1], 0.81), new PhoneCost(costBounds[1], costBounds[2], 0.92),
//                new PhoneCost(costBounds[2], costBounds[3], 0.96), new PhoneCost(costBounds[3], costBounds[4], 0.98),
//                new PhoneCost(costBounds[4], costBounds[5], 0.985), new PhoneCost(costBounds[5], costBounds[6], 1.0)
//        };
//        @Override
//        public Value generateValue(Object... params) {
//            boolean smart = (boolean) params[0];
...
//                    + (int) (Math.random()*(targetCosts[hit].getMaxCost() - targetCosts[hit].getMinCost()));
...
//
//    public MobilePhoneTest(int documentsCount, int requestsCount, int reqAttributesCount,
...
//        }
//
//        simpleEngineConfig = new EngineConfig(indexes,  bitSetSupplier, new TinyOptimizer());
//        currentArt = 0;
//    }
//
...
//                document.put(model.getAttributeName(), Arrays.asList(model.generateValue()));
//                document.put(color.getAttributeName(), Arrays.asList(color.generateValue()));
//                //TODO more complicated screen size generation
//                document.put(screenSize.getAttributeName(), Arrays.asList(screenSize.generateValue(minScreen, maxScreen)));
//
...
//                            Arrays.asList(operationSystem.generateValue(phoneManufactorer.getValue(), costCat)));
//                    int minRam = 128;
//                    int maxRam = 4096;
//                    document.put(ram.getAttributeName(), Arrays.asList(ram.generateValue(minRam, maxRam)));
//                } else {
//                    document.put(operationSystem.getAttributeName(),
...
//        return String.format("MobilePhoneTest [%s values, %s documents, %s requests]",
