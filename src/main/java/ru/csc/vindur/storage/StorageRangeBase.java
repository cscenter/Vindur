...
public abstract class StorageRangeBase<T extends Comparable<T>> extends
    ...
    public boolean checkValue(int docId, T value, RangeRequest request) {
        return value.compareTo((T) request.getLowBound()) >= 0
                && value.compareTo((T) request.getUpperBound()) <= 0;
    }
    ...
    public boolean validateRequestType(Object request) {
        ...
        return validateValueType(((RangeRequest) request).getLowBound())
                && validateValueType(((RangeRequest) request).getUpperBound());
    }

    public static final RangeRequest range(Object lowBound, Object upperBound) {
